...
public class PokemonBank {
	...
	public PokemonBank() throws IOException, ClassNotFoundException {
		 ...
		 */
                ...
                db= new Database(DB_FILE_NAME); // on créé un objet de type database
		server= new ServerSocket(SERVER_PORT, 1, InetAddress.getLocalHost()); //on créé un socket
	...
	public void handleClient() throws IOException, ClassNotFoundException {
		 ...
		 * - BankOperation
		 */
		ObjectInputStream ois=new ObjectInputStream(client.getInputStream()); // on créé un objectinputstream
		ObjectOutputStream oos=new ObjectOutputStream(client.getOutputStream()); // on créé un objectoutputstream
		...
		while (running) {
			...
			case LIST:
				...
				if (this.pokemons.size() == 0) {
					 ...
					 */
					oos.writeObject("Pas de pokemon dans la base de donnée");
                                        oos.flush();
				} else {
					/*
					 * TODO
			...
			case STORE:
				 ...
				 */
				oos.writeObject("Votre pokemon est safe");
                                oos.flush();
